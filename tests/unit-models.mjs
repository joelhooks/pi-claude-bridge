...
describe("models absent from pi-ai's snapshot", () => {
	...
	it("derives Opus 5.5 from Opus 5 and uses its measured bare 1M id", () => {
		const models = buildModels([mockPiAiModel("claude-opus-5")]);
		const derived = models.find((m) => m.id === "claude-opus-5-5");
		assert.ok(derived, "claude-opus-5-5 should be derived from claude-opus-5");
		assert.equal(derived.name, "Claude Opus 5.5");
		assert.equal(derived.maxTokens, 128000, "uses the measured Opus 5.5 output limit");
		assert.deepEqual(resolveClaudeCodeRuntimeModel(derived.id, PRO), {
			cliModelId: "claude-opus-5-5", contextWindow: 1000000,
		});
	});
});
